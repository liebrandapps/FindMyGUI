...
def icloud_login_mobileme(ctx, second_factor='sms'):
...
def gsa_authenticate(username, password, ctx, second_factor='sms'):
    anisetteUrl = ctx.cfg.general_anisetteHost + ":" + str(ctx.cfg.general_anisettePort)
    # Password is None as we'll provide it later
    usr = srp.User(username, bytes(), hash_alg=srp.SHA256, ng_type=srp.NG_2048)
    _, A = usr.start_authentication()

    r = gsa_authenticated_request({"A2k": A, "ps": ["s2k", "s2k_fo"], "u": username, "o": "init"}, anisetteUrl)
    ...
    r = gsa_authenticated_request({"c": r["c"], "M1": M, "u": username, "o": "complete"}, anisetteUrl)
...
"""
    ...
    if "au" in r["Status"] and r["Status"]["au"] in ["trustedDeviceSecondaryAuth", "secondaryAuth"]:
        ctx.log.debug("2FA required, requesting code")
        ...
        if second_factor == 'sms':
            success = sms_second_factor(spd["adsid"], spd["GsIdmsToken"], ctx)
...
def gsa_authenticated_request(parameters, anisetteUrl):
    body = {
        "Header": {"Version": "1.0.1"},
        "Request": {"cpd": generate_cpd(anisetteUrl)},
    }
...
def generate_cpd(anisetteUrl):
    ...
    cpd.update(generate_anisette_headers(anisetteUrl))
    return cpd


def generate_anisette_headers(anisetteUrl):
    try:
        import pyprovision
...
def trusted_second_factor(dsid, idms_token, ctx):
    anisetteUrl = ctx.cfg.general_anisetteHost + ":" + str(ctx.cfg.general_anisettePort)
